<?php	
	...
	// Name:		SiteBase CMS
	// Description:	A basic includes/XHTML/CSS Content Managment System for hand coding.
...
//	include ('common.php');
	...
	$cnt_author = '<div id="cnt_author">'.PHP_EOL;
	$cnt_author .= "\t".'<p>'.$cnt_author_name.'<br />'.PHP_EOL;
	$cnt_author .= "\t\t".$cnt_author_street.'<br />'.PHP_EOL;
	$cnt_author .= "\t\t".$cnt_author_city.', '.$cnt_author_state.' '.$cnt_author_code.'</p>'.PHP_EOL;

	$cnt_author .= "\t".'<p>Phone: '.$cnt_author_phone.'<br />'.PHP_EOL;
//	$cnt_author .= "\t\t".'Fax: '.$cnt_author_fax.'<br />'.PHP_EOL;
//	$cnt_author .= "\t\t".'Mobile: '.$cnt_author_mobile.'<br />'.PHP_EOL;
//	$cnt_author .= "\t\t".'E-mail: <a href="mailto:'.$cnt_ste_email.'">'.$cnt_ste_email.'</a></p>'.PHP_EOL;
	$cnt_author .= '</div>'.PHP_EOL;
...
//	$cnt_var[] = array('birthdate', 'Birthdate', '1975-01-31', 8, false);
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST'){
		...
		$cnt_sender_address = $cnt_sender_street.' '.$cnt_sender_apt.PHP_EOL;
		...
		// content plain
		$cnt_content_plain = 'Subject: '.$cnt_subject.PHP_EOL;
//		$cnt_content_plain .= 'From: '.$cnt_sender.PHP_EOL;
		$cnt_content_plain .= 'To: '.$cnt_recipient.PHP_EOL;
		$cnt_content_plain .= 'Date: '.$cnt_date."\n\n";

		$cnt_content_plain .= 'Company: '.$cnt_sender_company.PHP_EOL;
...
//		$cnt_content_plain .= 'Fax: '.$cnt_sender_fax.PHP_EOL;
//		$cnt_content_plain .= 'Mobile: '.$cnt_sender_mobile."\n\n";
//		$cnt_content_plain .= 'URL: '.$cnt_sender_url.PHP_EOL;
//		$cnt_content_plain .= 'Birthdate: '.$cnt_sender_birthdate.PHP_EOL;
		$cnt_content_plain .= 'Message:'.PHP_EOL;
		$cnt_content_plain .= $cnt_sender_text."\n\n";

		$cnt_content_plain .= 'referr: '.$cnt_sender_referr."\n\n";

		// content xhtml
		$cnt_content_html = "\t".'<p><strong>Subject:</strong> '.$cnt_subject.'</p>'.PHP_EOL;
		$cnt_content_html .= "\t".'<p><strong>From:</strong> '.$cnt_sender_name.' &lt;<a href="mailto:'.$cnt_sender_email.'">e-mail</a>&gt;<br />'.PHP_EOL;
		$cnt_content_html .= "\t\t".'<strong>To:</strong> '.$cnt_recipient_name.' &lt;<a href="mailto:'.$cnt_recipient_email.'">e-mail</a>&gt;<br />'.PHP_EOL;
		$cnt_content_html .= "\t\t".'<strong>Date:</strong> '.$cnt_date.'</p>'."\n\n";

		$cnt_content_html .= "\t".'<p><strong>Company:</strong> '.$cnt_sender_company.'<br />'.PHP_EOL;
		$cnt_content_html .= "\t\t".'<strong>Name:</strong> '.$cnt_sender_name.'<br />'.PHP_EOL;
//		$cnt_content_html .= "\t\t".'<strong>Address:</strong> '.$cnt_sender_street.$cnt_sender_apt.'<br />'.PHP_EOL;
//		$cnt_content_html .= "\t\t".$cnt_sender_city.', '.$cnt_sender_state.' '.$cnt_sender_code.'</p>'.PHP_EOL;
		$cnt_content_html .= "\t".'<p><strong>Phone:</strong> '.$cnt_sender_phone.'<br />'.PHP_EOL;
//		$cnt_content_html .= "\t\t".'<strong>Mobile:</strong> '.$cnt_sender_mobile.'<br />'.PHP_EOL;
//		$cnt_content_html .= "\t\t".'<strong>Fax:</strong> '.$cnt_sender_fax.'</p>'.PHP_EOL;
//		$cnt_content_html .= "\t".'<p><strong>URL:</strong> '.$cnt_sender_url.'</p>'.PHP_EOL;
//		$cnt_content_html .= "\t".'<p><strong>Birthdate:</strong> '.$cnt_sender_birthdate.'</p>'.PHP_EOL;
		$cnt_content_html .= "\t".'<p><strong>Message:</strong> <span class="contact_preview_text">'.$cnt_sender_text.'</span></p>'.PHP_EOL;

		$cnt_content_html .= "\t".'<p><strong>referr:</strong> '.$cnt_sender_referr.'</p>'.PHP_EOL;

		// content xhtml wrapper
		$cnt_html = '<!DOCTYPE html PUBLIC'.PHP_EOL;
		$cnt_html .= "\t".'"-//W3C//DTD XHTML 1.0 Strict//EN"'.PHP_EOL;
		...
		$cnt_html .= '<head>'."\n\n";
		$cnt_html .= "\t".'<meta http-equiv="content-type" content="'.$ste_incoding.'" />'.PHP_EOL;
		$cnt_html .= "\t".'<meta http-equiv="content-style-type" content="text/css" />'."\n\n";
		$cnt_html .= "\t".'<title>'.$cnt_subject.'</title>'."\n\n";
		$cnt_html .= "\t".'<link rel="stylesheet" type="text/css" title="e-mail" href="'.$ste_url.'/styles/e-mail.css" />'."\n\n";
		$cnt_html .= '</head>'."\n\n";
		$cnt_html .= '<body>'."\n\n";
		$cnt_html .= $cnt_content_html."\n\n";
		$cnt_html .= '</body>'."\n\n";
		$cnt_html .= '</html>'.PHP_EOL;

		// create e-mail -------------------------------------------- //
		// header
		$cnt_header = 'MIME-Version: 1.0'.PHP_EOL;
		$cnt_header .= 'To: '.$cnt_recipient.PHP_EOL;
		$cnt_header .= 'From: '.$cnt_sender.PHP_EOL;
//		$cnt_header .= 'Cc: '.$cnt_sender.PHP_EOL;
		$cnt_header .= 'Reply-To: '.$cnt_sender.PHP_EOL;
		$cnt_header .= 'Date: '.$cnt_date.PHP_EOL;
		$cnt_header .= 'Subject: '.$cnt_subject.PHP_EOL;
		$cnt_header .= 'Content-Type: multipart/mixed; boundary="{'.$cnt_content_mime_boundary.'}"'.PHP_EOL;
		$cnt_header .= 'Content-Transfer-Encoding: 7bit'.PHP_EOL;
		$cnt_header .= 'Content-Description:'."\n\n";
		$cnt_header .= 'This is a multi-part message in MIME format.'."\n\n";

		// plain message
		$cnt_body = '--'.$cnt_content_mime_boundary.PHP_EOL;
		$cnt_body .= 'Content-Type: '.$email_incoding.PHP_EOL;
		$cnt_body .= 'Content-Transfer-Encoding: 7bit'.PHP_EOL;
		$cnt_body .= $cnt_content_plain."\n\n";

		// xhtml message
		$cnt_body .= '--'.$cnt_content_mime_boundary.PHP_EOL;
		$cnt_body .= 'Content-Type: '.$email_incoding.PHP_EOL;
		$cnt_body .= 'Content-Transfer-Encoding: base64'.PHP_EOL;
		$cnt_body .= $cnt_html."\n\n";
		...
		if ($cnt_attach){
			...
			foreach ($_FILES['file']['error'] as $key => $error){
				...
				if ($error == UPLOAD_ERR_OK){
					...
					$cnt_body .= '--'.$cnt_content_mime_boundary.PHP_EOL;
					$cnt_body .= 'Content-Type: '.$type.'; name="'.$name.'"'.PHP_EOL;
					$cnt_body .= 'Content-Transfer-Encoding: base64'.PHP_EOL;
					$cnt_body .= 'Content-Disposition: attachment; filename="'.$name.'"'.PHP_EOL;
					$cnt_body .= $data."\n\n";
...
<?php
			...
			if ($status == 'send'){
				...
				if (mail($cnt_recipient_email, $cnt_subject, $cnt_body, $cnt_header)){
					...
					echo '<p class="contact_preview_sent"><strong>Notice:</strong> Your message has been sent.</p>'.PHP_EOL;
					echo '<p class="contact_preview_sent">Thank You.<p>'.PHP_EOL;

				} else {

					echo '<p class="contact_preview_warning\"><strong>Warning:</strong> Your message could not be sent. Please contact the site administrator.<br /><br />'.PHP_EOL;
					echo "\t".'<a href="mailto:'.$cnt_recipient_email.'">'.$cnt_recipient_email.'</a></p>'.PHP_EOL;
...
<?php
				...
				echo '<p class="contact_preview_notice"><strong>Preview:</strong> Check your message for errors.<br />'.PHP_EOL;
				echo "\t".'Click <em>Back</em> to edit or <em>Submit</em> to send.</p>'.PHP_EOL;

				$cnt_preview = $tab.'<div class="contact_preview">'.PHP_EOL;
				$cnt_preview .= $tab.$cnt_content_html;
				$cnt_preview .= $tab.'</div>'.PHP_EOL;
...
<?php
		...
		echo $tab.'<input type="hidden" name="status" value="preview" />'.PHP_EOL;

		// display recipient select options
		if ($cnt_multi){

			echo $tab.'<p><label>Recipient<span> *</span><br />'.PHP_EOL;
			echo $tab."\t".'<select id="contact_recipient_name" name="contact_recipient_name">'.PHP_EOL;
			...
			foreach ($cnt_multi_name as $k => $v){
				...
				echo $tab."\t\t".'<option value="'.$recipient_item.'">'.$recipient_name.'</option>'.PHP_EOL;

			}

			echo $tab."\t".'</select></label></p>'.PHP_EOL;

		}
		
		// cc sender
//		echo '<p><label><input type="checkbox" id="sender_copy" value="yes" />Send yourself a copy of this e-mail.</label></p>'.PHP_EOL;
...
//				name="contact_sender_'.$id.'"
		...
		if ($cnt_attach && $i = 1){
			...
			while ($i <= $cnt_attach_limit){
				...
				echo $tab.'<p id="attachment'.$i.'"><label>Attachment #'.$i.'<br />'.PHP_EOL;
				echo $tab."\t".'<input type="hidden" name="max_file_size" value="'.$cnt_attach_size.'" />'.PHP_EOL;
				echo $tab."\t".'<input type="file" name="file[]" class="form_input_file" /></label></p>'.PHP_EOL;
		...
		echo $tab.'<p>* Requried</p>'.PHP_EOL;
		echo $tab.'<p class="fc"><input type="submit" value="Preview" class="form_button" />'.PHP_EOL;
		echo $tab."\t".'<input type="reset" value="Reset" class="form_button" /></p>'.PHP_EOL;
