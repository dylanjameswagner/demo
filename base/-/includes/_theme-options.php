...
function twentyeleven_admin_enqueue_scripts( $hook_suffix ) {
	wp_enqueue_style( 'twentyeleven-theme-options', get_template_directory_uri() . '/-/styles/theme-options.css', false, '2011-04-28' );
	wp_enqueue_script( 'twentyeleven-theme-options', get_template_directory_uri() . '/-/scripts/theme-options.js', array( 'farbtastic' ), '2011-06-10' );
...
function twentyeleven_theme_options_init() {
	...
	add_settings_field(
		...
		__( 'Color Scheme', 'rockpaperrecords' ), // Setting field label
		'twentyeleven_settings_field_color_scheme', // Function that renders the settings field
		'theme_options', // Menu slug, used to uniquely identify the page; see twentyeleven_theme_options_add_page()
		'general' // Settings section. Same as the first argument in the add_settings_section() above
	);

	add_settings_field( 'link_color', __( 'Link Color',     'rockpaperrecords' ), 'twentyeleven_settings_field_link_color', 'theme_options', 'general' );
	add_settings_field( 'layout',     __( 'Default Layout', 'rockpaperrecords' ), 'twentyeleven_settings_field_layout',     'theme_options', 'general' );
}
...
function twentyeleven_theme_options_add_page() {
	$theme_page = add_theme_page(
		__( 'Theme Options', 'rockpaperrecords' ),   // Name of page
		__( 'Theme Options', 'rockpaperrecords' ),   // Label in menu
...
function twentyeleven_theme_options_help() {
	...
	$help = '<p>' . __( 'Some themes provide customization options that are grouped together on a Theme Options screen. If you change themes, options may change or disappear, as they are theme-specific. Your current theme, Twenty Eleven, provides the following Theme Options:', 'rockpaperrecords' ) . '</p>' .
			'<ol>' .
				'<li>' . __( '<strong>Color Scheme</strong>: You can choose a color palette of "Light" (light background with dark text) or "Dark" (dark background with light text) for your site.', 'rockpaperrecords' ) . '</li>' .
				'<li>' . __( '<strong>Link Color</strong>: You can choose the color used for text links on your site. You can enter the HTML color or hex code, or you can choose visually by clicking the "Select a Color" button to pick from a color wheel.', 'rockpaperrecords' ) . '</li>' .
				'<li>' . __( '<strong>Default Layout</strong>: You can choose if you want your site&#8217;s default layout to have a sidebar on the left, the right, or not at all.', 'rockpaperrecords' ) . '</li>' .
			'</ol>' .
			'<p>' . __( 'Remember to click "Save Changes" to save any changes you have made to the theme options.', 'rockpaperrecords' ) . '</p>';

	$sidebar = '<p><strong>' . __( 'For more information:', 'rockpaperrecords' ) . '</strong></p>' .
		'<p>' . __( '<a href="http://codex.wordpress.org/Appearance_Theme_Options_Screen" target="_blank">Documentation on Theme Options</a>', 'rockpaperrecords' ) . '</p>' .
		'<p>' . __( '<a href="http://wordpress.org/support/" target="_blank">Support Forums</a>', 'rockpaperrecords' ) . '</p>';
	...
	if ( method_exists( $screen, 'add_help_tab' ) ) {
		...
		$screen->add_help_tab( array(
			'title' => __( 'Overview', 'rockpaperrecords' ),
...
function twentyeleven_color_schemes() {
	$color_scheme_options = array(
		'light' => array(
			'value' => 'light',
			'label' => __( 'Light', 'rockpaperrecords' ),
			'thumbnail' => get_template_directory_uri() . '/-/includes/images/light.png',
			'default_link_color' => '#1b8be0',
		),
		'dark' => array(
			'value' => 'dark',
			'label' => __( 'Dark', 'rockpaperrecords' ),
			'thumbnail' => get_template_directory_uri() . '/-/includes/images/dark.png',
...
function twentyeleven_settings_field_link_color() {
	...
	<input type="button" class="pickcolor button hide-if-no-js" value="<?php esc_attr_e( 'Select a Color', 'rockpaperrecords' ); ?>" />
	<div id="colorPickerDiv" style="z-index: 100; background:#eee; border:1px solid #ccc; position:absolute; display:none;"></div>
	<br />
	<span><?php printf( __( 'Default color: %s', 'rockpaperrecords' ), '<span id="default-color">' . twentyeleven_get_default_link_color( $options['color_scheme'] ) . '</span>' ); ?></span>
	<?php
}
...
function twentyeleven_theme_options_render_page() {
	...
	<div class="wrap">
		<?php screen_icon(); ?>
		<h2><?php printf( __( '%s Theme Options', 'rockpaperrecords' ), get_current_theme() ); ?></h2>
		<?php settings_errors(); ?>
